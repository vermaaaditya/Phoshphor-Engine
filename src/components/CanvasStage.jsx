...
import { RAMP_PRESETS, indicesToFrame, mapLuminanceToIndices } from '../lib/asciiMapper';
import { createDistortionState, resizeDistortionState, updateDistortionField } from '../lib/distortionEngine';
...
export default function CanvasStage({
  ...
  const distortionEngineRef = useRef({
    cursor: { active: false, col: -1, row: -1 },
    state: createDistortionState(0),
    applyForce(col, row) {
      this.cursor = { active: true, col, row };
    },
    clearForce() {
      this.cursor = { active: false, col: -1, row: -1 };
    },
    update({ cols, rows, strength, preset, time }) {
      this.state = resizeDistortionState(this.state, cols * rows);
      updateDistortionField({
        ...
        cursor: this.cursor,
        settings: {
          radius: Math.max(2.5, Math.min(cols, rows) * 0.12),
          recovery: 0.84,
          strength,
        },
        preset,
  ...
  useEffect(() => {
    ...
    const render = (time) => {
      ...
      const distortionState = distortionEngineRef.current.update({
        cols: asciiGrid.cols,
        rows: asciiGrid.rows,
        strength: distortionStrength / 100,
      ...
      const lines = indicesToFrame(
        ...
        asciiGrid.rows,
        asciiGrid.ramp,
        distortionState.shift,
      );
      const cellWidth = width / asciiGrid.cols;
      const cellHeight = height / asciiGrid.rows;
      const fontSize = Math.max(8, Math.min(cellHeight * 1.04, cellWidth * 1.9));
      ...
      for (let row = 0; row < asciiGrid.rows; row += 1) {
        ...
        for (let col = 0; col < asciiGrid.cols; col += 1) {
          ...
          context.fillText(
            line[col],
            col * cellWidth + distortionState.x[index] * cellWidth * 0.18,
            row * cellHeight + distortionState.y[index] * cellHeight * 0.18,
          );
        }
      }
  ...
  return (
    <section ref={stageRef} className="flex-grow bg-surface-container-lowest relative overflow-hidden flex items-center justify-center">
      ...
      <canvas
        ...
        onMouseLeave={() => distortionEngineRef.current.clearForce()}
        onMouseMove={(event) => {
          if (!sourceImage || !asciiGrid) {
            return;
          }
